...
[![Npm](https://img.shields.io/npm/v/notifly-sdk.svg)](https://www.npmjs.com/package/notifly-sdk)
...
(권장사항) 로그아웃 시에는 `setUserId(null)`을 호출하여 기기와 유저의 연결을 해지해 주세요.
(권장사항) 회원 탈퇴 시에는 [Delete Users API](/api-reference/users-events/delete-users)를 구현하여 유저 정보를 완전히 삭제해 주세요.
...
#### 유저 ID 해제
로그아웃 시에는 `setUserId(null)`을 호출하여 유저 등록을 해제하는 것이 권장됩니다. 이 호출은 현재 기기와 유저 간의 연결을 끊고, 관련된 유저 데이터(유저 프로퍼티, 캠페인 피로도, 이벤트 로그 등)를 삭제합니다.
